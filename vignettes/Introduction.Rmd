...
So, we have a dataset of 30 samples, 2 conditions, 6 replicates each with 5 time points. The timepoints in the data are aligned. 
...
data1.res<-RolDE(data=data1, des_matrix=des_matrix1, doPar=T, n_cores=3)
```
...
The calculation of significance values in RolDE is controlled via the parameters *estSigVal* and *sigValSampN* which control if the significance values should be calculated and how many permutations should be used when determining the significance values. Computational time required by RolDE can be reduced by not calculating the significance values by setting *estSigVal* to FALSE. By increasing *sigValSampN*, the significance values can be estimated more accurately but the required computational time will also increase. If parallel processing for RolDE is enabled, it will also be utilized when estimating the significance values. 
...
In addition to the random data already explored, semi-simulated spike-in datasets with differential expression between the conditions are also included in the RolDE package:
...
All the datasets included in the RolDE package are briefly described in the variable *data_descriptions*

```{r data included in RolDE, echo=TRUE}
...
data3.res<-RolDE(data=data3, des_matrix=des_matrix3, doPar=T, n_cores=3, sig_adj_meth = "bonferroni")
```
...
How many of the proteins remain signifcant at the alpha level of 0.05 after the Bonferroni procedure to control for multiple hypothesis testing:
...
The design matrix for data with non aligned time points is assembled similarly:

```{r explore data5, echo=TRUE}
data(data5)
data("des_matrix5")
dim(data5)
dim(des_matrix5)
...
Instead of fixed time points, the integer values in the time point column 3 are now replaces with continuous numerical values, or time values. In this example data, column 3 consists of age of the patients at the time of the sampling. 

Running RolDE with the default settings and parallel processing for data5:

```{r Run RolDE for data5, echo=TRUE, eval=FALSE}
data5.res<-RolDE(data=data5, des_matrix=des_matrix5, aligned = F, doPar=T, n_cores=7)
```
...
Parameter *min_comm_diff* controls how many common time points must two replicates (individuals) have in different conditions to be compared. The first value controls the number of common time points for the RegROTS module, while the second one controls the number of common time points for the DiffROTS module. If *min_comm_diff* is set to "auto", RolDE will use a value of 3 for the RegROTS module and a value of 1 for the DiffROTS module.In the case of data with non-aligned time points, the first value of *min_comm_diff* controls how many time values (or similar, e.g. age, temperature) must both replicates in different conditions have in the common time interval to be compared. 
...
*min_ind_timep* determines the minimum number of time points/values all individuals included in the analysis must have. If any individual has less time points than *min_ind_timep*, RolDE will throw an error. Such replicates must be removed from the analysis. Any replicate or individual cannot have less than 3 time points/values to be included in the analysis.

*min_feat_obs* controls the number of non-missing values a feature must have for an individual in a condition to be compared in the RegROTS module and the DiffROTS module (in data with aligned time points). A feature is required to have at least *min_feat_obs* non-missing values for both replicates in the different conditions to be compared. If lowered, more missing values are allowed but the analysis may become less accurate. If increased, only replicates with less missing values for a feature are compared. 

The user can control the degree of polynomials used by the RegROTS and the PolyReg modules via the *degtree_RegROTS* and the *degree_PolyReg parameters*. If left to "auto", RolDE will by default use as the *degree_RegROTS*=max(1, min(floor(median(t)/2),4)) and as the *degree_PolyReg*=max(2, min((median(t)-1),5)), where t is a vector of the number of time points/values in all the replicates.
...
data1.res<-RolDE(data=data1, des_matrix=des_matrix1, doPar=T, n_cores=3, degree_RegROTS = 2, degree_PolyReg = 3)
```

*na_treat* controls the treatment of missing values when calculating rank products. The default option "last" will rank features with missing scores last and RolDE will produce a final rank product for all the features in the data. If *na_treat* is set to "keep", missing scores are retained as missing when calculating internal rank products and some features might receive a missing final rank product if enough values are missing for the feature. 

By default, RolDE uses fixed effects only regression with a common intercept and slopes for the replicates/individuals in the PolyReg module and in the DiffROTS module (in data with non-aligned time points). However, the user can choose to use mixed effects regression modelling when appropriate, by setting the parameter *polyregType* as "mixed". The desired level of random effects for the mixed models can be set with the parameter *mix_degree*. When *polyregType* is set to "mixed" and *mix_degree* is set to 0 (the default), a random intercept for each replicate will be used for the polynomial regression models in the PolyReg module and the DiffROTS module (in data with non-aligned time points). Alternatively, a random effect also for the linear term (slope) for each replicate can be allowed by setting the *mix_degree* to 1.
...
data1.res<-RolDE(data=data1, des_matrix=des_matrix1, doPar=T, n_cores=3, polyregType = "mixed", mix_degree = 0)
```

Analyzing data5 using mixed effects modelling with random intercepts and also random linear slopes for the replicates in the PolyReg AND the DiffROTS modules of RolDE:

```{r change RolDE parameters 4, echo=TRUE, eval=FALSE}
data5.res<-RolDE(data=data5, des_matrix=des_matrix5, aligned = F, doPar=T, n_cores=7, polyregType = "mixed", mix_degree = 1)
```

Altering the *polyregType* parameter has only an effect for the DiffROTS module, when the time points in the data are non-aligned. In non-aligned data, the expression level differences between the conditions in the DiffROTS module is examined when accounting for time-associated trends of varying complexity in the data. 
...
### Example of the preparation of data from paired measurements for RolDE

(Note: the following section can be removed. I thought I would provide a simple example on how to adjust paired data to enable the analysis of such data in the unpaired mode in RolDE.)

As mentioned, the data needs to be adjusted for possible confounding factors / effects prior to entering RolDE. For example, if the data is from paired measurements of replicates/individuals, the data can be adjusted for this effect prior to entering RolDE.

Let's make data3 paired so that replicates 1,2 and 3 measured in the different conditions are considered as paired measurements:

```{r create paired data, echo=TRUE}
data.pair=data3
for(i in 1:nrow(data.pair)){
  data.pair[i,c(grep("r2", colnames(data.pair)))]=data.pair[i,c(grep("r2", colnames(data.pair)))]+rnorm(mean = 0.5, sd=0.05, n = 1)
  data.pair[i,c(grep("r3", colnames(data.pair)))]=data.pair[i,c(grep("r3", colnames(data.pair)))]+rnorm(mean = 1, sd=0.05, n = 1)
}
```

Now the measurements for replicate 2 ("r2") in both conditions are separated by a constant from the measurements from replicate 1 ("r1"), simulating a slight batch effect resulting from paired experiments or processing of the samples in a paired manner. This effect is unique to each protein, so the constant difference is also unique to each protein. The same is true for replicate 3 ("r3"); measurements for replicate 3 are separated from the measurements of replicate 2 and replicate 1 by different constants factors, varying for each protein. 

Let's see how this affects the measurements for an example spike-in protein:

```{r plot paired data, echo=TRUE, fig.width=8, fig.height=4}

head(rownames(data3))

#Look at protein "P02768ups|ALBU_HUMAN_UPS;CON__P02768-1" as an example

par(mfrow=c(1,2))

plot(as.numeric(data3[2,c(grep("r1", colnames(data3)[1:15]))]), type = "o", col="1", ylim = c(range(data3[2,])), main="Unpaired data3", ylab = "Expression", xlab = "Timepoint")
lines(as.numeric(data3[2,c(grep("r2", colnames(data3)[1:15]))]), type = "o", col="2")
lines(as.numeric(data3[2,c(grep("r3", colnames(data3)[1:15]))]), type = "o", col="3")

lines(as.numeric(data3[,16:30][2,c(grep("r1", colnames(data3)[16:30]))]), type = "o", col="1", lty=3)
lines(as.numeric(data3[,16:30][2,c(grep("r2", colnames(data3)[16:30]))]), type = "o", col="2", lty=3)
lines(as.numeric(data3[,16:30][2,c(grep("r3", colnames(data3)[16:30]))]), type = "o", col="3", lty=3)

#Paired data3:
plot(as.numeric(data.pair[2,c(grep("r1", colnames(data.pair)[1:15]))]), type = "o", col="1", ylim = c(range(data.pair[2,])), main="Paired data3", ylab = "Expression", xlab = "Timepoint")
lines(as.numeric(data.pair[2,c(grep("r2", colnames(data.pair)[1:15]))]), type = "o", col="2")
lines(as.numeric(data.pair[2,c(grep("r3", colnames(data.pair)[1:15]))]), type = "o", col="3")

lines(as.numeric(data.pair[,16:30][2,c(grep("r1", colnames(data.pair)[16:30]))]), type = "o", col="1", lty=3)
lines(as.numeric(data.pair[,16:30][2,c(grep("r2", colnames(data.pair)[16:30]))]), type = "o", col="2", lty=3)
lines(as.numeric(data.pair[,16:30][2,c(grep("r3", colnames(data.pair)[16:30]))]), type = "o", col="3", lty=3)
```

Replicates in the data are colored with different colors, and the expression of replicates in Condition 1 are presented with a solid line while the expression of the corresponding replicates in Condition 2 are presented with a dotted line. 

From the above figure, we can observe that the replicates are now expressed at slightly different levels in the paired data, simulating a slight batch effect resulting from paired processing of the samples in the data. 

However, we can adjust for such a pair effect with simple linear models:

```{r adjust paired data, echo=TRUE}
data.adj=matrix(nrow = nrow(data.pair), ncol=ncol(data.pair))
colnames(data.adj)=colnames(data.pair)
rownames(data.adj)=rownames(data.pair)

for(i in 1:nrow(data.adj)){
  temp=data.frame(matrix(nrow = ncol(data.pair), ncol=2))
  temp[,1]=as.numeric(data.pair[i,])
  temp[,2]=as.factor(rep(c(1,2,3), 10))
 
  colnames(temp)=c("x", "pair")
  rownames(temp)=colnames(data.pair)
  
  mod=lm(x~as.numeric(pair), data=temp)
  res=mod$residuals
  res=res+mean(as.numeric(temp[,1]), na.rm = T)
  
  data.adj[i,]=res
}
```

Now, Let's see how well we were able to adjust for the pair effect:

```{r plot adjusted data, echo=FALSE, fig.width=8, fig.height=4}
...
#Paired data3 adjusted for the paired effect:
plot(as.numeric(data.adj[2,c(grep("r1", colnames(data.adj)[1:15]))]), type = "o", col="1", ylim = c(range(data.adj[2,])), main = "Adjusted paired data3", ylab = "Expression", xlab = "Timepoint")
lines(as.numeric(data.adj[2,c(grep("r2", colnames(data.adj)[1:15]))]), type = "o", col="2")
lines(as.numeric(data.adj[2,c(grep("r3", colnames(data.adj)[1:15]))]), type = "o", col="3")

lines(as.numeric(data.adj[,16:30][2,c(grep("r1", colnames(data.adj)[16:30]))]), type = "o", col="1", lty=3)
lines(as.numeric(data.adj[,16:30][2,c(grep("r2", colnames(data.adj)[16:30]))]), type = "o", col="2", lty=3)
lines(as.numeric(data.adj[,16:30][2,c(grep("r3", colnames(data.adj)[16:30]))]), type = "o", col="3", lty=3)

```

In this case, a simple linear model was sufficient to adjust for the pair effect, but often more complicated (mixed) models may be required to adjust for such effects (e.g. in the presence of other effects/confounding factors). 

RolDE can now be applied in a standard way to analyze the paired data adjusted for the pair effect:

```{r Run RolDE for adjusted paired data, echo=TRUE, eval=FALSE}
res<-RolDE(data=data.adj, des_matrix=des_matrix3, aligned = T, doPar=T, n_cores=3)
```
...
Välikangas T, Suomi T, ELo LL, et al. Enhanced longitudinal differential expression detection in proteomics with robust reproducibility optimization regression.
