DiffROTSAligned<-function(data, des_matrix, min_comm_diff, min_feat_obs, rots_runs, B_for_ROTS, K_for_ROTS, rand_seed, na_treat, n_cores, weigh_runs){
  ...
  rots_res_frame <- foreach::foreach(run=1:length(rots_runs), .export = c("fillGaps_New", "fillGapsAll_New"), .packages = c("ROTS", "doParallel", "matrixStats", "foreach"), .combine=cbind) %dopar% {
    ...
    for(r in 1:nrow(diff_frame)){ #so we need to remove those that have too many nas in group 1 or 2
    ...
    for(r in 1:nrow(diff_frame)){
      ...
      diff_frame[r,]<-fillGaps_New(all_quant_vals = all_quant_vals, row = row, groups_for_rots = groups_for_rots, rand_seed = seed_for_imp)
    }
    ...
    #determine K
    if(K_for_ROTS=="auto"){
      K_for_ROTS<-nrow(diff_new)/4
    }
    suppressMessages(expr = {rots_out<-ROTS::ROTS(data = diff_new, groups = groups_for_rots, B = B_for_ROTS, K = K_for_ROTS, paired = F, seed = rand_seed, progress = F)})
