#Organize the aligned data properly for RolDE_Aligned and run some validity checks.

validateAlignedData<-function(data, des_matrix, min_comm_diff, min_ind_timep, min_feat_obs, B_for_ROTS, K_for_ROTS, rand_seed, na_treat, n_cores, polyregType, mix_degree, estSigVal, sigValSampN, sig_adj_meth, add_refGroup, iter_polyDegree, weigh_runs){
  ...
  if(min_comm_diff!="auto"){
    if(!is.numeric(min_comm_diff)){stop("Incorrect min_comm_diff. Numerical values (two) needed. See intructions.")}
  ...
  names(return_list)<-c("Data", "Design_Matrix", "Test_Type", "Min_Comm_Diff")
  return(return_list)
}
