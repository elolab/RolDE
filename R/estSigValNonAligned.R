estSigValNonAligned<-function(data, res_regrots, res_diffrots, res_polyreg, reg_rots_pval, diff_rots_pval, polyreg_pval, rank_products, na_treat, rand_seed, n_cores, sig_adj_meth, sigValSampN){
  ...
  sim_rps <- foreach::foreach(co=1:ncol(sim_rps), .export=c("getSimRankProdsNonAligned", "getSPPoly", "getSimPDif", "getSimRPs", "getRank"), .packages = c("foreach"), .combine=cbind) %dopar% {
    ...
    col_Val<-getSimRankProdsNonAligned(reg_rots_pval, diff_rots_pval, polyreg_pval, all_poly_pvals, ord_poly, sim_p, rp_reg, p_diff, p_poly, rand_seed, na_treat, sigValSampN)
    col_Val
  }
