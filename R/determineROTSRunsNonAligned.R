determineROTSRunsNonAligned<-function(data, des_matrix, test_type, min_comm_diff){
  ...
  if(test_type=="unpaired"){
    ...
    max_inds_in_one_run<-min(length(inds_condition1), length(inds_condition2))
    ...
    #is there an easier way perhaps? Perhaps think later, works now.

  }#Removed run determination for paired and blocked design, if needed look from older versions.
