estSigValAligned<-function(data, res_regrots, res_diffrots, res_polyreg, reg_rots_pval, diff_rots_pval, polyreg_pval, rank_products, na_treat, rand_seed, n_cores, sig_adj_meth, sigValSampN){
  ...
  #ranks_all<-cbind(rank_reg, rank_diff, rank_poly)
  ...
  sim_rps <- foreach::foreach(co=1:ncol(sim_rps), .export=c("getSimRankProds", "getSPPoly", "getSimRPs", "getRank"), .packages = c("foreach"), .combine=cbind) %dopar% {
    ...
    col_Val<-getSimRankProds(reg_rots_pval, diff_rots_pval, polyreg_pval, all_poly_pvals, ord_poly, sim_p, rp_reg, rp_diff, p_poly, used_seed, na_treat, sigValSampN)
    col_Val
  }
