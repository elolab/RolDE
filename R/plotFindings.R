...
plotFindings<-function(file_name=NULL, RolDE_res, top_n, col1="blue", col2="red"){
  ...
  colnames(des_plot)=c("Sample", "Condition", "Time", "Replicate")
  des_plot=data.frame(des_plot)
  ...
  range_time=range(as.numeric(as.character(des_plot$Time)), na.rm = T)
  range_time=c((range_time[1]-range_time[1]*0.1), (range_time[2]+range_time[1]*0.1))
  #Need to be adjusted?

  #plot each top feature
  if(!is.null(file_name)){
    pdf(file=file_name, width = 11.7, height = 8.3,onefile = T)
  }

  for(j in 1:length(features_to_plot)){
    range_exprs=range(data_plot[features_to_plot[j],c(des_plot$Sample)], na.rm = T)
